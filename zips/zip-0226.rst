...
- v1 Signature Digest: the per-Action-Group signature digest defined in `v1 Signature Digest`_. This is used in certain cases to provide spend authorization for the Actions of a single Action Group. 
...
In the `OrchardZSA Transaction Structure`_, there is now an $\mathsf{assetBurn}$ set for each Action Group. 
...
TODO: 
The multiple Action Groups in a single transaction may each contain overlapping entries for a single Asset (that is, multiple Action Groups ask to burn the same Asset). 
This should be allowed(?), but we need to specify how $\mathsf{MAX\_BURN\_VALUE}$ applies in this case.
...
TODO: In the equation above, needs to be a union of AssetBurn sets across all Action Groups.
