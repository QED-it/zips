...
2. The issuer identifier, denoted by $\mathsf{issuer}$ is the identifier of a particular Issuer.
   This identifier is used to validate the issuance of Asset Identifiers by a given issuer, and is used by all blockchain users (specifically the owners of notes for that Asset, and consensus validators) to associate the Asset in question with the issuer.
