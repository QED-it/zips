...
The ZSA Protocol adds the following keys to the key components [#protocol-addressesandkeys]_:
...
We instantiate the issuance authorization signature scheme :math:`\mathsf{IssueAuthSig}` as a Schnorr signature over the :math:`\mathtt{secp256k1}` curve.
We define the constants as per the :math:`\mathtt{secp256k1}` standard parameters, as described in BIP 340 [#bip-0340]_. 
...
where :math:`\mathbb{B}^{\mathbb{Y}^{[k]}}` denotes the set of sequences of :math:`k` bytes, and :math:`\mathbb{B}^{\mathbb{Y}^{[\mathbb{N}]}}` denotes the type of byte sequences of arbitrary length, as defined in the protocol specification [#protocol-notation]_.
...
The signing algorithm :math:`\mathsf{IssueAuthSig.Sign}` is defined as the `Sign` algorithm from BIP 340 [#bip-0340]_, except for the following changes:

* We use :math:`\mathsf{hash}_{\mathsf{ZIP0227/aux}}` in place of :math:`\mathsf{hash}_{\mathsf{BIP0340/aux}}`.
* We fix the auxiliary random data :math:`a = \mathtt{[0x00]}^{32}`.
* We use :math:`\mathsf{hash}_{\mathsf{ZIP0227/nonce}}` in place of :math:`\mathsf{hash}_{\mathsf{BIP0340/nonce}}`.
* We use :math:`\mathsf{hash}_{\mathsf{ZIP0227/challenge}}` in place of :math:`\mathsf{hash}_{\mathsf{BIP0340/challenge}}`.

The validating algorithm :math:`\mathsf{IssueAuthSig.Validate}` is defined as the `Verify` algorithm from BIP 340 [#bip-0340]_, except for the following changes:
