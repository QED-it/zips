...
**Note:** To simplify notations, we use :math:`\mathsf{AssetBase}^{\mathsf{Protocol}}` as an alias for :math:`\mathsf{AssetBase}^{\mathsf{Protocol}}_{\mathsf{AssetId}}`. Likewise, when the protocol is clear from the context we may simply use :math:`\mathsf{AssetBase}`.
