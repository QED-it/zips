...
For an issuance master key :math:`\mathsf{imk} \in \mathbb{B}^{\mathbb{Y}[32]}`, and a message :math:`M \in \mathbb{B}^{\mathbb{Y}^{[\mathbb{N}]}}`, we define :math:`\mathsf{IssueAuthSig.Sign}(\mathsf{imk}, M)` as:

* Let the auxiliary data :math:`a = \mathtt{[0x00]}^{32}`.
* Return :math:`\mathsf{Sign}(\mathsf{imk},M)`.

The validating algorithm :math:`\mathsf{IssueAuthSig.Validate}` is defined using the :math:`\mathsf{Verify}` algorithm from BIP 340 [#bip-0340]_.
For an issuance validating key :math:`\mathsf{ik} \in \mathbb{B}^{\mathbb{Y}^{[32]}}`, a message :math:`M \in \mathbb{B}^{\mathbb{Y}^{[\mathbb{N}]}}`, and a signature :math:`\sigma \in \mathbb{B}^{\mathbb{Y}^{[64]}}`, we define :math:`\mathsf{IssueAuthSig.Sign}(\mathsf{ik}, M, \sigma)` as:
