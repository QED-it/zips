...
In ``zcashd``, this key is derived in a manner similar to the generation of the Orchard master key in ZIP 32 [#zip-0032-orchard-master]_, as detailed in this section.

Let :math:`S` be a seed byte sequence of a chosen length, which MUST be at least 32 bytes and at most 252 bytes.

- Calculate :math:`I = \textsf{BLAKE2b-512}(\texttt{"ZIP32ZSAIssuance"},S)`.
- Split :math:`I` into two 32-byte sequences :math:`I_L` and :math:`I_R`.
- Return :math:`(\mathsf{sk}_m, \mathsf{c}_m) := (I_L, I_R)` as the master extended issuance key, :math:`m_{\mathsf{Issuance}}`.
...
We adapt the notation of ZIP 32 [#zip-0032]_ for shielded HD paths, using prime marks (:math:`'`) to
indicate hardened derivation (:math:`i' = i + 2^{31}`) as in BIP 44 [#bip-0044]_:

- :math:`\mathsf{CDKik}(\mathsf{CDKik}(\mathsf{CDKik}(m_\mathsf{Issuance}, a), b), c)` is written as :math:`m_\mathsf{Issuance} / a / b / c`.

We define the issuance master key path as :math:`m_\mathsf{Issuance} / purpose' / coin\_type' / account'`. We fix the path levels as follows:
...
- :math:`account`: fixed to index :math:`0`. Defined as in BIP 44 [#bip-0044]_.

This key path gives us an "extended" issuance master key :math:`(\mathsf{sk}, \mathsf{c})`, from which we set the issuance master key to be :math:`\mathsf{sk_{iss}} := \mathsf{sk}`.
