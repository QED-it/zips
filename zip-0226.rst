...
We assume :math:`n` Actions in a transfer. Out of these :math:`n` Actions, we further distinguish (for the sake of verbosity and clarity) between Actions related to ZEC and Actions related to Custom Assets. We assume :math:`l` Actions related to ZEC and :math:`n-l` actions related to Custom Assets, where :math:`l \in [0,n]`.
...
We denote :math:`|\mathsf{assetBurn}|` the cardinality of the :math:`\mathsf{assetBurn}` set.
...
As for the value balance verification, we assume :math:`n` Actions, :math:`l` of which are related to ZEC and the remaining :math:`n-l` Actions related to Custom Assets.

The burn mechanism verification is equivalent to:

.. math:: \mathsf{bvk} = (\mathsf{(\sum_{i=1}^{l} cv_i^{net})} + \mathsf{(\sum_{j=l+1}^{n} cv_i^{net})}) - ([\mathsf{v^{balanceOrchard}}]\mathcal{V}^{\mathsf{Orchard}} + [0]\mathcal{R}^{\mathsf{Orchard}}) - (\sum_{k=1}^{|\mathsf{assetBurn}|} [\mathsf{v_{k}^{AssetBase}}]\mathsf{AssetBase_{k}} + [0]\mathcal{R}^{\mathsf{Orchard}})
