...
We include the ability to pause the ZSA functionality, via a :math:`\mathsf{enable\_zsa}` boolean flag. When this flag is set to false, it is not possible to perform transactions involving Custom Assets (the circuit will fail).
...
The :math:`\mathsf{enable\_zsa}` Flag
`````````````````````````````````````

The following constraints must be added to disable transactions involving Custom Assets when the :math:`\mathsf{enable\_zsa}` flag is set to false:

- if :math:`\mathsf{enable\_zsa}` is not activated (i.e. :math:`\mathsf{enable\_zsa} = 0`), then constrain :math:`\mathsf{is\_native\_asset} = 1`, since the :math:`\mathsf{AsssetBase}` must be equal to the native asset.
