...
This allows us to maintain a single commitment instance for the old note commitment, that will be used both for pre-ZSA Orchard notes and for ZSA notes. The pre-ZSA Orchard notes will be conserved and not actually converted into ZSA notes, as we will always need to compute them.
However, new notes will always be ZSA notes with an Asset Base (even if they are for ZEC and not a Custom Asset).
...
A Split Input is an exact copy of a previously issued input note (that is, a note that has previously been included in the Merkle tree), with the following changes:
...
When the number of input notes of a particular Asset Base is smaller than the required number of output notes for the same Asset Base, the sender creates Split Inputs for the same Asset Base. Note that we do not care about whether the previously issued note copied to create a Split Input is owned by the sender, or whether it was nullified before.
...
Asset Identifier Equality
'''''''''''''''''''''''''

The following constraints must be added to ensure that the input and output note are of the same :math:`\mathsf{AssetId}`:
...
Burn Mechanism
==============
The burn mechanism may be needed for off-boarding the Wrapped Assets from the chain, or enabling advanced tokenomics on Assets. It is part of the Issuance/Burn protocol, but given that it can be seen as an extension of the Transfer protocol, we describe it here for readability.

The burn mechanism is a transparent extension to the transfer protocol that enables a specific amount of any Asset Identifier to be sent into “oblivion”. Our burn mechanism does NOT send Assets to a non-spendable address, it simply reduces the total number of units of a given Asset in circulation at the consensus level. It is enforced at the consensus level, by using an extension of the value balance mechanism used for ZEC Assets.

The sender includes a :math:`\mathsf{valueBurn_{AssetId}}` variable for every Asset Identifier that is being burnt. As we will show in the `ZSA Transaction Structure`_, this is separate from the regular :math:`\mathsf{valueBalance^Orchard}` that is the default transparent value for the ZEC Asset.
We require that for every :math:`\mathsf{valueBurn_{AssetId}}` provided as above by the sender, :math:`\mathsf{valueBurn_{AssetId}} \neq 0`. This is enforced via a consensus rule.

For every Custom Asset that is burnt, we add to the :math:`\mathsf{assetBurn}` vector the tuple :math:`(\mathsf{AssetId},\mathsf{valueBurn_{AssetId}})` such that the validator of the transaction can compute the value commitment with the corresponding value base point of that Asset. This ensures that the values are all balanced out with respect to the Asset Identifiers in the transfer.

.. math:: \mathsf{assetBurn} = \{ (\mathsf{AssetId},\mathsf{valueBurn_{AssetId}})\ |\ \forall\ \mathsf{AssetId}\ \textit{s.t.}\ \mathsf{valueBurn_{AssetId}} \neq 0 \}
...
+---------------+------------+-------------------------+-----------------------------------------------------------------------------------+
| 40*nAssetBurn | vAssetBurn | AssetBurnV6[nAssetBurn] | A sequence of Asset Burn descriptions, encoded per `ZSA Asset Burn Description`_. |
+---------------+------------+-------------------------+-----------------------------------------------------------------------------------+
...
A ZSA Asset Burn description is encoded in a transaction as an of a AssetBurnV6 type:

+-------+-----------+-----------------------------+---------------------------------------------------------------------------------------------------------------------------+
| Bytes | Name      | Data Type                   | Description                                                                                                               |
+=======+===========+=============================+===========================================================================================================================+
| 32    | AssetType | byte[32]                    | For the Orchard-based ZSA protocol, this is the encoding of the Asset Base :math:`\mathsf{AssetBase}^{\mathsf{Orchard}}`. |
+-------+-----------+-----------------------------+---------------------------------------------------------------------------------------------------------------------------+
...
- The notes generated via the ZSA protocol are different from those generated via the Orchard protocol. It will be possible to distinguish between notes generated via these two protocols. However, all ZEC notes will be fully spendable with the ZSA protocol transaction structure due to the built-in backward compatibility.
