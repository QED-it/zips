...
In order to be able to pause the ZSA functionality, we added the :math:`\mathsf{enable\_zsa}` boolean flag. When this flag is set to false, it is not possible to perform ZSA transactions (the circuit will fail).
...
The following constraints must be added to disable ZSA transactions when the :math:`\mathsf{enable\_zsa}` flag is set to false:

- if :math:`\mathsf{enable\_zsa}` is not activated (i.e. :math:`\mathsf{enable\_zsa} = 0`), then the :math:`\mathsf{AsssetBase}` must be equal to the native asset (i.e. :math:`\mathsf{is\_native\_asset} = 1`).
