...
- Asset: A type of note that can be transferred on the Zcash blockchain, identified by the :math:`\mathsf{AssetId}` parameter.
...
In order to be able to represent different Assets, we need to define a data field that uniquely represents the Asset in question, which we call the Asset Identifier, denoted :math:`\mathsf{AssetId}`.
This Asset Identifier maps to an Asset Base :math:`\mathsf{AssetBase}^{\mathsf{Orchard}}_{\mathsf{AssetId}}` that is stored in Orchard-based ZSA notes.
...
Most of the protocol is kept the same as the Orchard protocol released with NU5, except for the protocol changes proposed in this ZIP.
Unless specified otherwise, we follow the notations of the Zcash Orchard protocol specifications [#protocol-specs]_.
...
where :math:`\mathbb{P}, \ell_{\mathbb{P}}, q_{\mathbb{P}}` are as defined for the Pallas curve [#protocol-pallasandvesta]_, and :math:`\mathsf{NoteCommit}^{\mathsf{Orchard}}.\mathsf{Trapdoor}, \mathsf{NoteCommit}^{\mathsf{Orchard}}.\mathsf{Output}` are as defined in the Zcash protocol specification [#protocol-abstractcommit]_.
...
:math:`\mathsf{bsk} = \sum_{i=1}^{n} \mathsf{rcv_{i}}`.

Then the verifier MUST compute

.. math:: \mathsf{bvk = (\sum_{i=1}^{n} cv_i^{net})}  - \mathsf{ ValueCommit_0^{Orchard}(v^{balanceOrchard})} = \sum_{i=1}^{n} \mathsf{rcv_{i}^{net}}\mathcal{R}^{\mathsf{Orchard}}
...
.. math:: \mathsf{assetBurn} = \{ (\mathsf{v^{AssetBase}}, \mathsf{AssetBase})\ |\ \forall\ \mathsf{AssetBase}\ \textit{s.t.}\ \mathsf{v^{AssetBase}} \neq 0 \}
...
.. math:: \mathsf{bvk = (\sum_{i=1}^{n} cv_i^{net})}  - \mathsf{ ValueCommit_0^{Orchard}(v^{balanceOrchard})} - \sum_{\mathsf{assetBurn}} \mathsf{ValueCommit_0^{OrchardZSA}(v^{AssetBase}, AssetBase) }
...
.. [#protocol-specs] `Zcash Protocol Specification, Version 2022.3.8. <protocol/protocol.pdf>`_
