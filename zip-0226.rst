...
The transaction digest algorithm defined in ZIP 244 [#zip-0244]_ is modified by the ZSA protocol to add a new branch for issuance information, and add an additional hash for the Asset Base within the ``orchard_digest``. The details of these changes are described in this section, and highlighted using the ``[UPDATED FOR ZSA]`` or ``[ADDED FOR ZSA]`` text label. We omit the details of the sections that do not change for the ZSA protocol. 

The overall tree structure of the hash is as follows::
    ...
    │   └── valueBalance
    └── orchard_digest
    │   ├── orchard_actions_compact_digest            [UPDATED FOR ZSA]
    │   ├── orchard_actions_memos_digest              [UPDATED FOR ZSA]
    │   ├── orchard_actions_noncompact_digest         [UPDATED FOR ZSA]
    │   ├── flagsOrchard
    │   ├── valueBalanceOrchard
    │   └── anchorOrchard
    └── issuance_digest                               [ADDED FOR ZSA]
        ├── issue_actions_digest                      [ADDED FOR ZSA]
        └── issuanceValidatingKey                     [ADDED FOR ZSA]
...
in the hash::
   ...
   T.4a.iv : encCiphertext[..84]  (First 52 bytes of field encoding)  [UPDATED FOR ZSA]
