...
- It should incentivize the miners to accept the transaction into the blocks they propose.

We believe the payment of fees in ZEC is the best approach for the ecosystem. We discuss the other options in the `Fee Considerations`_ subsection of the `Other Considerations`_ section.
...
In the P2P setting, or more generally, in situations where the party performing the matching is known to the parties creating the Swap Orders, it is possible to include balanced Actions with output notes for the matcher so as to pay fees to the matcher.

In order to support cases where the matcher may not be known in advance, we note that the Action Group structure allows for the creation of a (positively) value-imbalanced “fee action” where traders pass an input ZEC note to the fee action to “deposit” a fee in the order.
The Matcher would need to insert a new Action Group whose overall value balances out with the other Action Groups in the bundle - to form the finalized Zcash transaction. 
This Action Group appended by the Matcher would contain one "fee-taking" action.
This comes at the expense of having an additional AG in the finalized Zcash tx (which proof won't be aggregated with the other proofs).
...
Some questions about fee capture are as follows. 
Is it acceptable if the fees are paid in the Assets being swapped? 
Should it be paid by both parties involved in the Swap?

Paying fees in ZEC helps alleviate concerns about other Assets piggy-backing on the Zcash network without providing value to holders of ZEC.
